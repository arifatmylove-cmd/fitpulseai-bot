package com.fitpulseai.builder

import android.content.Context
import com.fitpulseai.network.ApiClient
import com.fitpulseai.network.GitHubClient
 ...
 *
 * The generated app is a customized clone of the Fit Pulse AI codebase: same proven
...
class AppBuilderService(private val context: Context) {
    ...
    companion object {
        ...
        private const val TEMPLATE_REPO  = "fitpulseai-bot"
    ...
    suspend fun build(
        ...
        for (entry in tree) {
            ...
            val destPath = srcPath.replace("com/fitpulseai/", "com/${spec.packageSuffix}/")
    ...
    private fun applySubstitutions(content: String, spec: AppSpec): String {
        ...
        return content
            ...
            .replace("package com.fitpulseai", "package com.${spec.packageSuffix}")
            .replace("import com.fitpulseai", "import com.${spec.packageSuffix}")
            .replace("com.fitpulseai", "com.${spec.packageSuffix}")
            // Human-readable name
            .replace("Fit Pulse AI", spec.appName)
            .replace("Fit Pulse AI", spec.appName)
            .replace("fitpulseai-bot", spec.packageSuffix + "-bot")
            .replace("fitpulseai", spec.packageSuffix)
            // SharedPreferences key so data doesn't bleed between apps
            .replace("Fit Pulse AIPrefs", "${safeName}Prefs")
            // Icon letter (in ic_launcher_foreground.xml)
            .replace(">F<", ">${spec.iconLetter}<")
            // Brand colour
            .replace("#FF4500", spec.primaryColor)
            // System prompt
            .replace(ApiClient.DEFAULT_SYSTEM_PROMPT, spec.systemPrompt)
            // Workflow artifact name
            .replace("fitpulseai-debug-apk", "${spec.packageSuffix}-debug-apk")
            .replace("Build Fit Pulse AI APK", "Build ${spec.appName} APK")
    }
}
